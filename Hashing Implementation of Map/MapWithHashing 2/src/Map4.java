...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private void createNewRep(int hashTableSize) {
        ...
        for (int i = 0; i < hashTableSize; i++) {
            this.hashTable[i] = new Map2<K, V>(); //Set each bucket to be Map2 object
        }
    ...
    public final void add(K key, V value) {
        ...
        int bucket = mod(key.hashCode(), this.hashTable.length); //Find a bucket
        this.hashTable[bucket].add(key, value); //Add it to the bucket
    ...
    public final Pair<K, V> remove(K key) {
        ...
        int bucket = mod(key.hashCode(), this.hashTable.length); //Find a bucket
        Pair<K, V> removed = this.hashTable[bucket].remove(key); //Remove it from the bucket
    ...
    public final Pair<K, V> removeAny() {
        ...
        //Finds the first bucket that has at least one element inside
        while (this.hashTable[i].size() < 1) {
            i++;
        }
        this.size -= 1; //Decrease Size
