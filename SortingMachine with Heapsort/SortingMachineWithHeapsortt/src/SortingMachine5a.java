...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        int left = 2 * top + 1; //left
        int right = left + 1; //right

        //right subtree boundary
        if (right <= last) {
            //left is less than top
            ...
            //right is less than top
            else if (order.compare(array[right], array[left]) <= 0
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        if (left <= array.length - 1) { //heapify left
            heapify(array, left, order);
            if (right <= array.length - 1) { //heapify right
                heapify(array, right, order);
            }
        }

        siftDown(array, top, array.length - 1, order); //sift
    }
